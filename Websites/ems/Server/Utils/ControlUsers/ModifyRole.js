...
async function ModifyRole(modifierRole, emp_id, userRole, newRole, employee_user_email,failing_messages) {

        // Modidify Role
        if(modifierRole === "SuperAdmin"){
            const succeeded =await SuperAdmin.ChangeOtherUserRole(emp_id , userRole , newRole , employee_user_email)
            if(!succeeded){
